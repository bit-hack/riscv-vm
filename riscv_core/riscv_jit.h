...
static void gen_mov_r9_rsiindex(struct block_t *block,
                                struct riscv_t *rv,
                                uint32_t offset) {
  JITPRINTF("mov r9, [rsi + %u]\n", offset);
  gen_emit_data(block, rv, "\x4c\x8b\x8e", 3);
  gen_emit_data(block, rv, (uint8_t*)&offset, 4);
}
...
static void gen_call_rsiindex(struct block_t *block,
                              struct riscv_t *rv,
                              uint32_t offset) {
  JITPRINTF("call [rsi + %u]\n", offset);
  gen_emit_data(block, rv, "\xff\x96", 2);
  gen_emit_data(block, rv, (uint8_t*)&offset, 4);
}
